...
## Index fields (thin row)

- name
- purpose (when to use, one line)
- keywords (exact hits)
- do-not-use-when (hard veto)
- body pointer (file or page, not the body itself)
- references (load only if the body asks)
...
1. Match purpose and keywords against the current turn.
2. Drop any hit whose do-not-use-when or edge case matches this turn.
3. If two skills still match, load neither. Name both and why. A closer keyword does not cancel a second match.
4. Load the body, the matching edge cases, and only lessons tied to this skill or this kind of miss. Last few, not the archive.
...
6. If the store is down, use a local thin cache. No body. No guess. Locks still hold.

## Stores

Same fields, any store: files, Notion, Airtable, Coda, or a sheet. The store is what a human edits. Keep a small local copy of name, purpose, keywords, and negatives. Do not query the store on every message.
